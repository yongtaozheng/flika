...
// ── Config ──────────────────────────────────────────────────────────────────
const config = reactive<PixelWaterfallConfig>({
  pixelSize: 16,
  cascadeMode: 'columns',
  gravity: 1.2,
  drift: 0.5,
...
// ── Engine ──────────────────────────────────────────────────────────────────
const engine = usePixelWaterfallEngine(canvasRef, images, configRef, beats)
...
function handleImagesAdd(newImgs: UploadedImage[]) {
  const waterfallImgs: PixelWaterfallImage[] = newImgs.map((img) => ({
    ...img,
    spreadDuration: config.spreadDuration,
    pauseDuration: config.pauseDuration,
  }))
  images.value = [...images.value, ...waterfallImgs]
...
// ── Re-generate blocks when config changes ──────────────────────────────────
watch(
  () => [config.pixelSize, config.cascadeMode, config.gravity, config.drift, config.trailEnabled],
  () => {
...
const statusText = computed(() => {
  ...
  if (!isPlaying.value) {
    ...
    return `${images.value.length} 张图片 · ${config.pixelSize}px 像素块${mode}`
  }
  const info = engine.getPlaybackInfo(playElapsed.value)
  const modeTag = engine.isBeatMode() ? ' 🎵' : ''
  if (info.phase === 'spreading')
    return `第 ${info.index + 1}/${images.value.length} 张 · 瀑布中 ${Math.round(info.progress * 100)}%${modeTag}`
    ...
    <aside class="sidebar">
      ...
      <div class="sidebar-block">
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="4" max="48" step="2"
              v-model.number="config.pixelSize"
              :style="sliderFill(config.pixelSize, 4, 48)" />
            <span class="setting-val">{{ config.pixelSize }}px</span>
          </div>
        </div>
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="0.5" max="3" step="0.1"
              ...
              :style="sliderFill(config.gravity, 0.5, 3)" />
            <span class="setting-val">{{ config.gravity.toFixed(1) }}x</span>
          </div>
        </div>
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="0" max="1.5" step="0.1"
              v-model.number="config.drift"
              :style="sliderFill(config.drift, 0, 1.5)" />
            <span class="setting-val">{{ Math.round(config.drift * 100) }}%</span>
          </div>
        </div>

        <!-- Cascade mode -->
        <div class="mode-row">
          <label class="mode-label">级联模式</label>
          <div class="mode-btns">
            <button
              class="mode-btn"
              :class="{ active: config.cascadeMode === 'columns' }"
              @click="config.cascadeMode = 'columns'"
            >瀑布</button>
            <button
              class="mode-btn"
              :class="{ active: config.cascadeMode === 'rows' }"
              @click="config.cascadeMode = 'rows'"
            >水平</button>
            <button
              class="mode-btn"
              :class="{ active: config.cascadeMode === 'wave' }"
              @click="config.cascadeMode = 'wave'"
            >波浪</button>
            <button
              class="mode-btn"
              :class="{ active: config.cascadeMode === 'random' }"
              @click="config.cascadeMode = 'random'"
            >随机</button>
          </div>
        </div>
        ...
        <label class="loop-toggle">
          ...
          <span class="loop-label">拖尾效果</span>
        </label>
