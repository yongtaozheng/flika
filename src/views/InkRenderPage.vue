...
import type { UploadedImage, InkImage, InkConfig } from '../types'
import { useInkRenderEngine } from '../composables/useInkRenderEngine'
...
// ── Config ──────────────────────────────────────────────────────────────────
const config = reactive<InkConfig>({
  spreadDuration: 3000,
  pauseDuration: 1000,
  loop: false,
  edgeRoughness: 0.6,
  inkDensity: 0.85,
  splatterEnabled: true,
...
// ── Engine ──────────────────────────────────────────────────────────────────
const engine = useInkRenderEngine(canvasRef, images, configRef, beats)
...
function clearCanvas() {
  ...
  if (c) { c.fillStyle = 'rgb(245,238,225)'; c.fillRect(0, 0, CW, CH) }
}

// ── Image management ────────────────────────────────────────────────────────
function handleImagesAdd(newImgs: UploadedImage[]) {
  const inkImgs: InkImage[] = newImgs.map((img) => ({
    ...img,
    points: [],
    spreadDuration: config.spreadDuration,
    pauseDuration: config.pauseDuration,
  }))
  images.value = [...images.value, ...inkImgs]
...
const statusText = computed(() => {
  ...
  if (!isPlaying.value) {
    ...
    return `${images.value.length} 张图片 · ${totalPts} 个墨点${mode}`
  }
  const info = engine.getPlaybackInfo(playElapsed.value)
  const modeTag = engine.isBeatMode() ? ' 🎵' : ''
  if (info.phase === 'spreading')
    return `第 ${info.index + 1}/${images.value.length} 张 · 渲墨中 ${Math.round(info.progress * 100)}%${modeTag}`
    ...
    <aside class="sidebar">
      ...
      <div class="sidebar-block">
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="0.2" max="1" step="0.05"
              v-model.number="config.inkDensity"
              :style="sliderFill(config.inkDensity, 0.2, 1)" />
            <span class="setting-val">{{ Math.round(config.inkDensity * 100) }}%</span>
          </div>
        </div>
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="0" max="1" step="0.05"
              v-model.number="config.edgeRoughness"
              :style="sliderFill(config.edgeRoughness, 0, 1)" />
            <span class="setting-val">{{ Math.round(config.edgeRoughness * 100) }}%</span>
          </div>
        </div>

        <label class="loop-toggle">
          <input type="checkbox" v-model="config.inkWashBase" />
          <span class="loop-label">水墨底图</span>
        </label>

        <label class="loop-toggle">
          <input type="checkbox" v-model="config.splatterEnabled" />
          <span class="loop-label">墨水飞溅</span>
        </label>
...
.point-marker {
  ...
  border: 2px solid rgba(80, 60, 50, 0.8);
  cursor: pointer;
  display: flex; align-items: center; justify-content: center;
  transition: transform 0.15s, background 0.15s;
  z-index: 2;
  box-shadow: 0 0 8px rgba(30, 20, 40, 0.5);
}
.point-marker:hover {
  transform: scale(1.3);
  background: rgba(80, 40, 20, 0.7);
  border-color: rgba(120, 80, 50, 0.9);
  box-shadow: 0 0 12px rgba(80, 40, 20, 0.6);
}
.point-marker-num {
...
.status-dot.spreading { background: #6b5b4f; box-shadow: 0 0 6px #6b5b4f; animation: pulse 0.8s infinite; }
...
.auto-add-btn {
  ...
  font-size: 11.5px; font-weight: 500;
  color: #6b5b4f; background: rgba(107, 91, 79, 0.1);
  border: 1px solid rgba(107, 91, 79, 0.15);
  cursor: pointer; transition: all 0.15s;
}
.auto-add-btn:hover { background: rgba(107, 91, 79, 0.18); border-color: rgba(107, 91, 79, 0.3); }
...
.point-num {
  ...
  background: rgba(107, 91, 79, 0.15); color: #6b5b4f;
