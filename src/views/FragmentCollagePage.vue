...
import type { UploadedImage, FragmentImage, FragmentConfig } from '../types'
import { useFragmentCollageEngine } from '../composables/useFragmentCollageEngine'
...
// ── Images ──────────────────────────────────────────────────────────────────
const images = ref<FragmentImage[]>([])
...
// ── Config ──────────────────────────────────────────────────────────────────
const config = reactive<FragmentConfig>({
  gridCols: 5,
  gridRows: 4,
  scatterRange: 0.8,
  rotationRange: 45,
...
// ── Engine ──────────────────────────────────────────────────────────────────
const engine = useFragmentCollageEngine(canvasRef, images, configRef, beats)
...
function handleImagesAdd(newImgs: UploadedImage[]) {
  const fragImgs: FragmentImage[] = newImgs.map((img) => ({
    ...img,
    spreadDuration: config.spreadDuration,
    pauseDuration: config.pauseDuration,
  }))
  images.value = [...images.value, ...fragImgs]
...
// ── Re-generate fragments when config changes ───────────────────────────────
watch(
  () => [config.gridCols, config.gridRows, config.scatterRange, config.rotationRange,
         config.assembleMode, config.fragmentGap, config.perspectiveEnabled],
  () => {
...
const statusText = computed(() => {
  ...
  if (!isPlaying.value) {
    ...
    return `${images.value.length} 张图片 · ${config.gridCols}×${config.gridRows} 碎片${mode}`
  }
  const info = engine.getPlaybackInfo(playElapsed.value)
  const modeTag = engine.isBeatMode() ? ' 🎵' : ''
  if (info.phase === 'spreading')
    return `第 ${info.index + 1}/${images.value.length} 张 · 拼贴中 ${Math.round(info.progress * 100)}%${modeTag}`
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="2" max="10" step="1"
              v-model.number="config.gridCols"
              :style="sliderFill(config.gridCols, 2, 10)" />
            <span class="setting-val">{{ config.gridCols }}</span>
          </div>
        </div>
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="2" max="8" step="1"
              ...
              :style="sliderFill(config.gridRows, 2, 8)" />
            <span class="setting-val">{{ config.gridRows }}</span>
          </div>
        </div>
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="0.2" max="1.5" step="0.1"
              v-model.number="config.scatterRange"
              :style="sliderFill(config.scatterRange, 0.2, 1.5)" />
            <span class="setting-val">{{ Math.round(config.scatterRange * 100) }}%</span>
          </div>
        </div>
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="0" max="180" step="5"
              v-model.number="config.rotationRange"
              :style="sliderFill(config.rotationRange, 0, 180)" />
            <span class="setting-val">{{ config.rotationRange }}°</span>
          </div>
        </div>
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="0" max="10" step="1"
              v-model.number="config.fragmentGap"
              :style="sliderFill(config.fragmentGap, 0, 10)" />
            <span class="setting-val">{{ config.fragmentGap }}px</span>
          </div>
        </div>

        <!-- Assemble mode -->
        <div class="mode-row">
          <label class="mode-label">组装模式</label>
          <div class="mode-btns">
            <button
              class="mode-btn"
              :class="{ active: config.assembleMode === 'random' }"
              @click="config.assembleMode = 'random'"
            >随机</button>
            <button
              class="mode-btn"
              :class="{ active: config.assembleMode === 'radial' }"
              @click="config.assembleMode = 'radial'"
            >径向</button>
            <button
              class="mode-btn"
              :class="{ active: config.assembleMode === 'sweep' }"
              @click="config.assembleMode = 'sweep'"
            >扫描</button>
          </div>
        </div>

        <label class="loop-toggle">
          <input type="checkbox" v-model="config.perspectiveEnabled" />
          <span class="loop-label">3D 翻转效果</span>
        </label>
