...
import type { UploadedImage, RaindropErosionImage, RaindropErosionConfig } from '../types'
import { useRaindropErosionEngine } from '../composables/useRaindropErosionEngine'
...
// ── Images ──────────────────────────────────────────────────────────────────
const images = ref<RaindropErosionImage[]>([])
...
// ── Config ──────────────────────────────────────────────────────────────────
const config = reactive<RaindropErosionConfig>({
  dropDensity: 60,
  dropMaxRadius: 20,
  fallSpeed: 1.0,
  streakIntensity: 0.7,
...
// ── Engine ──────────────────────────────────────────────────────────────────
const engine = useRaindropErosionEngine(canvasRef, images, configRef, beats)
...
function handleImagesAdd(newImgs: UploadedImage[]) {
  const rImgs: RaindropErosionImage[] = newImgs.map((img) => ({
    ...img,
...
watch(
  () => [config.dropDensity, config.dropMaxRadius, config.erosionMode, config.dropHue],
  () => {
...
const statusText = computed(() => {
  ...
  if (!isPlaying.value) {
    const modeLabel = { natural: '自然', storm: '暴风', drizzle: '细雨' }[config.erosionMode]
    const mode = config.beatSyncEnabled && beats.value.length >= 2 ? ' · 踩点模式' : ''
    return `${images.value.length} 张图片 · ${modeLabel}模式${mode}`
  }
  const info = engine.getPlaybackInfo(playElapsed.value)
  const modeTag = engine.isBeatMode() ? ' 🎵' : ''
  if (info.phase === 'spreading')
    return `第 ${info.index + 1}/${images.value.length} 张 · 雨滴侵蚀中 ${Math.round(info.progress * 100)}%${modeTag}`
...
// ── Hue presets ──────────────────────────────────────────────────────────────
const huePresets = [
  { label: '水', hue: 0, color: 'hsl(200, 60%, 75%)' },
  { label: '蓝', hue: 210, color: 'hsl(210, 80%, 60%)' },
  { label: '青', hue: 180, color: 'hsl(180, 80%, 55%)' },
  { label: '紫', hue: 270, color: 'hsl(270, 70%, 65%)' },
  { label: '红', hue: 0, color: 'hsl(0, 80%, 60%)' },
  { label: '金', hue: 45, color: 'hsl(45, 90%, 55%)' },
]
// Fix: "水" uses dropHue=0 (default transparent water), "红" uses hue=355
huePresets[0].hue = 0
huePresets[4].hue = 355
    ...
    <aside class="sidebar">
      ...
      <div class="sidebar-block">
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="10" max="200" step="5"
              v-model.number="config.dropDensity"
              :style="sliderFill(config.dropDensity, 10, 200)" />
            <span class="setting-val">{{ config.dropDensity }}</span>
          </div>
        </div>
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="5" max="50" step="1"
              v-model.number="config.dropMaxRadius"
              :style="sliderFill(config.dropMaxRadius, 5, 50)" />
            <span class="setting-val">{{ config.dropMaxRadius }}px</span>
          </div>
        </div>
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="0.5" max="3" step="0.1"
              v-model.number="config.fallSpeed"
              :style="sliderFill(config.fallSpeed, 0.5, 3)" />
            <span class="setting-val">{{ config.fallSpeed.toFixed(1) }}x</span>
          </div>
        </div>
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="0" max="1" step="0.05"
              v-model.number="config.streakIntensity"
              :style="sliderFill(config.streakIntensity, 0, 1)" />
            <span class="setting-val">{{ Math.round(config.streakIntensity * 100) }}%</span>
          </div>
        </div>
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="0" max="30" step="1"
              v-model.number="config.splashCount"
              :style="sliderFill(config.splashCount, 0, 30)" />
            <span class="setting-val">{{ config.splashCount }}</span>
          </div>
        </div>

        <!-- Erosion mode -->
        <div class="mode-row">
          <label class="mode-label">侵蚀模式</label>
          <div class="mode-btns">
            <button
              class="mode-btn"
              :class="{ active: config.erosionMode === 'natural' }"
              @click="config.erosionMode = 'natural'"
            >
            ...
            <button
              class="mode-btn"
              :class="{ active: config.erosionMode === 'storm' }"
              @click="config.erosionMode = 'storm'"
            >
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <button
              class="mode-btn"
              :class="{ active: config.erosionMode === 'drizzle' }"
              @click="config.erosionMode = 'drizzle'"
            >
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <line x1="8" y1="19" x2="8" y2="21" /><line x1="12" y1="18" x2="12" y2="20" />
                <line x1="16" y1="19" x2="16" y2="21" />
                <path d="M4 15.67A5 5 0 0 1 9 7h1.26A8 8 0 1 1 20 15.67" />
              </svg>
              细雨
            </button>
          </div>
        ...
        <!-- Hue presets -->
        <div class="hue-row">
          <label class="hue-label">雨滴颜色</label>
          <div class="hue-presets">
            <button
              v-for="preset in huePresets"
              :key="preset.label"
              class="hue-btn"
              :class="{ active: config.dropHue === preset.hue }"
              :style="{ '--hue-color': preset.color }"
              @click="config.dropHue = preset.hue"
              :title="preset.label"
            >
        ...
        <div class="setting-row" style="margin-top: 8px;">
          ...
          <div class="setting-ctl">
            ...
            <span class="setting-val">{{ config.dropHue }}°</span>
          </div>
        </div>
