...
import type { UploadedImage, RadialBeamImage, RadialBeamConfig } from '../types'
import { useRadialBeamEngine } from '../composables/useRadialBeamEngine'
...
// ── Images ──────────────────────────────────────────────────────────────────
const images = ref<RadialBeamImage[]>([])
...
// ── Config ──────────────────────────────────────────────────────────────────
const config = reactive<RadialBeamConfig>({
  beamCount: 8,
  rotationSpeed: 1.5,
  glowIntensity: 0.7,
  beamWidth: 0.6,
...
// ── Engine ──────────────────────────────────────────────────────────────────
const engine = useRadialBeamEngine(canvasRef, images, configRef, beats)
...
function handleImagesAdd(newImgs: UploadedImage[]) {
  const beamImgs: RadialBeamImage[] = newImgs.map((img) => ({
    ...img,
    spreadDuration: config.spreadDuration,
    pauseDuration: config.pauseDuration,
  }))
  images.value = [...images.value, ...beamImgs]
...
watch(
  () => [config.beamCount, config.beamWidth, config.centerX, config.centerY, config.beamHue],
  () => {
...
// ── Canvas click to set beam center ──────────────────────────────────────────
...
const statusText = computed(() => {
  ...
  if (!isPlaying.value) {
    ...
    return `${images.value.length} 张图片 · ${config.beamCount} 束光束${mode}`
  }
  const info = engine.getPlaybackInfo(playElapsed.value)
  const modeTag = engine.isBeatMode() ? ' 🎵' : ''
  if (info.phase === 'spreading')
    return `第 ${info.index + 1}/${images.value.length} 张 · 光束扫描中 ${Math.round(info.progress * 100)}%${modeTag}`
...
const huePresets = [
  ...
  { label: '紫', hue: 270, color: 'hsl(270, 80%, 65%)' },
      ...
      <div class="sidebar-block">
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="3" max="24" step="1"
              v-model.number="config.beamCount"
              :style="sliderFill(config.beamCount, 3, 24)" />
            <span class="setting-val">{{ config.beamCount }}</span>
          </div>
        </div>
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="0" max="4" step="0.1"
              ...
              :style="sliderFill(config.rotationSpeed, 0, 4)" />
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="0.1" max="1" step="0.05"
              v-model.number="config.beamWidth"
              :style="sliderFill(config.beamWidth, 0.1, 1)" />
            <span class="setting-val">{{ Math.round(config.beamWidth * 100) }}%</span>
          </div>
        </div>
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="0" max="1" step="0.05"
              ...
              :style="sliderFill(config.glowIntensity, 0, 1)" />
            <span class="setting-val">{{ Math.round(config.glowIntensity * 100) }}%</span>
          </div>
        </div>

        <!-- Hue presets -->
        <div class="hue-row">
          <label class="hue-label">光束颜色</label>
          <div class="hue-presets">
            <button
              v-for="preset in huePresets"
              :key="preset.hue"
              class="hue-btn"
              :class="{ active: config.beamHue === preset.hue }"
              :style="{ '--hue-color': preset.color }"
              @click="config.beamHue = preset.hue"
              :title="preset.label"
            >
        ...
        <div class="setting-row" style="margin-top: 8px;">
          ...
          <div class="setting-ctl">
            ...
            <span class="setting-val">{{ config.beamHue }}°</span>
          </div>
        </div>
...
.center-dot {
  ...
  border-radius: 50%;
  background: rgba(150, 180, 255, 0.8);
...
.status-dot.spreading { background: var(--amber); box-shadow: 0 0 6px var(--amber); animation: pulse 0.8s infinite; }
