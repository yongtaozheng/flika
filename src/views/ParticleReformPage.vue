...
import type { UploadedImage, ParticleImage, ParticleConfig } from '../types'
import { useParticleReformEngine } from '../composables/useParticleReformEngine'
...
// ── Images ──────────────────────────────────────────────────────────────────
const images = ref<ParticleImage[]>([])
...
// ── Engine ──────────────────────────────────────────────────────────────────
const engine = useParticleReformEngine(canvasRef, images, configRef, beats)
...
function handleImagesAdd(newImgs: UploadedImage[]) {
  ...
  images.value = [...images.value, ...particleImgs]
...
// ── Re-generate particles when config changes ───────────────────────────────
watch(
  () => [config.particleSize, config.scatterRange, config.scatterMode,
         config.trailEnabled, config.glowEnabled, config.hueShift],
  () => {
...
const statusText = computed(() => {
  ...
  if (!isPlaying.value) {
    ...
    return `${images.value.length} 张图片 · ${config.particleSize}px 粒子${mode}`
  }
  const info = engine.getPlaybackInfo(playElapsed.value)
  const modeTag = engine.isBeatMode() ? ' 🎵' : ''
  if (info.phase === 'spreading')
    return `第 ${info.index + 1}/${images.value.length} 张 · 重组中 ${Math.round(info.progress * 100)}%${modeTag}`
      ...
      <div ref="canvasShellRef" class="canvas-shell" :class="{ 'is-fullscreen': isFullscreen }">
        ...
        <div v-if="images.length === 0" class="empty-state">
          <svg width="44" height="44" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.2" opacity="0.25">
            <circle cx="5" cy="5" r="1.5" /><circle cx="12" cy="3" r="1" /><circle cx="19" cy="6" r="1.5" />
            <circle cx="3" cy="12" r="1" /><circle cx="12" cy="12" r="2" /><circle cx="21" cy="12" r="1" />
            <circle cx="6" cy="19" r="1.5" /><circle cx="12" cy="21" r="1" /><circle cx="18" cy="18" r="1.5" />
          </svg>
          <span>添加图片后预览粒子重组效果</span>
        </div>
      </div>
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="2" max="12" step="1"
              ...
              :style="sliderFill(config.particleSize, 2, 12)" />
            <span class="setting-val">{{ config.particleSize }}px</span>
          </div>
        </div>
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="0.3" max="2" step="0.1"
              ...
              :style="sliderFill(config.scatterRange, 0.3, 2)" />
        ...
        <div class="setting-row">
          ...
          <div class="setting-ctl">
            <input type="range" min="0" max="360" step="5"
              ...
              :style="sliderFill(config.hueShift, 0, 360)" />
            <span class="setting-val">{{ config.hueShift }}°</span>
          </div>
        </div>

        <!-- Scatter mode -->
        <div class="mode-row">
          <label class="mode-label">散开模式</label>
          <div class="mode-btns">
            <button
              class="mode-btn"
              :class="{ active: config.scatterMode === 'explode' }"
              @click="config.scatterMode = 'explode'"
            >爆炸</button>
            <button
              class="mode-btn"
              :class="{ active: config.scatterMode === 'vortex' }"
              @click="config.scatterMode = 'vortex'"
            >漩涡</button>
            <button
              class="mode-btn"
              :class="{ active: config.scatterMode === 'gravity' }"
              @click="config.scatterMode = 'gravity'"
            >重力</button>
            <button
              class="mode-btn"
              :class="{ active: config.scatterMode === 'radial' }"
              @click="config.scatterMode = 'radial'"
            >径向</button>
          </div>
        </div>

        <label class="loop-toggle">
          <input type="checkbox" v-model="config.trailEnabled" />
          <span class="loop-label">粒子拖尾</span>
        </label>

        <label class="loop-toggle">
          <input type="checkbox" v-model="config.glowEnabled" />
          <span class="loop-label">粒子发光</span>
        </label>
